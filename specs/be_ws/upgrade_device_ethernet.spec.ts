...
import {FIRMWARE_VERSION, firmwareVersionConfigB7} from "../../ws/FirmwareVersionConfig";
        ...
        try {
            await Timeouts.raceError(async () => {
                ...
                for (const version of prevVersionList) {
                    ...
                    const versionCode = activeVersion.config.version;
                    //todo parse from XX_76_01 -> 76.01

                    await Timeouts.raceError(
                        wsInstance.getSubscribedObjectData(
                            "update", 'panelSettings', "version",
                            versionCode, "substring",
                        ), {
                            awaitSeconds: 3 * 60
                        })
