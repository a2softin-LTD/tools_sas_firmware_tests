...
1. Use and update test data file in the project: ``resources/test_data.json``:
   ...
   3. save the file
2. Run the command in the console: ```node tests-generator.js```
3. In the project folder ```specs/global/autogenerated``` you will see automatically generated test files. Note that each file contains a test for only ```one``` device with the corresponding user data. For example, if you have 2 users specified in the json file, and the first has 2 devices, and the second has 3 devices, as a result, 5 test files will be generated
4. Run the command in the console: ```npx playwright test --grep "@sas_upgrade_async_single_user_multiple_devices_json"```


- Example:
```
npx playwright test --grep "@sas_upgrade_async_single_user_multiple_devices_json"
```
use following tags: `@sas_upgrade_async_single_user_multiple_devices_json`
