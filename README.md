...
1. Use and update test data file in the project: ``resources/test_data.json``:
   ...
   3. save the file
2. Run the command in the console: ```node generators/upgrade-firmware-tests-generator.js```
...
4. Count the number of files generated (this is necessary to run tests with the appropriate number of workers - WORKER_NUMBER)
...
## 6. `Create/delete reactions in parallel for all specified devices`
####
Steps to run tests:
1. Use and update test data file in the project: ``resources/test_data.json``:
   1. the file contains an array of user test data in the form of separate Objects, eg:
   ...
   2. update the file with your details: 
      1. use the field of the reaction array
      2. for each device we use the corresponding element of the reaction array, eg: 
      ```"deviceIdsHex": ["00:08:B7:10:02:08", "00:08:B7:00:00:08"] -> "reactions": [3, 5]```
      3. so, for the "00:08:B7:10:02:08" device we will create '3' reactions, respectively for the "00:08:B7:00:00:08" - '5' reactions
   3. save the file
2. Run the command in the console: ```node generators/reactions-tests-generator.js```
3. In the project folder ```specs/global/autogenerated/create_delete_reactions``` you will see automatically generated test files. Note that each file contains a test for only ```one``` device with the corresponding user data. For example, if you have 2 users specified in the json file, and the first has 2 devices, and the second has 3 devices, as a result, 5 test files will be generated
4. Count the number of files generated (this is necessary to run tests with the appropriate number of workers - WORKER_NUMBER)
5. It is important to know that each file generates 2 autotests: the first one is for creating reactions, the second one is for deleting reactions. Accordingly, the test for creating will be launched first, and then for deleting
6. Run the command in the console (creating reactions): ```npx playwright test --grep "@sas_reactions_async_single_user_single_device_create_json" --workers={WORKER_NUMBER}```
7. Run the command in the console (deleting reactions): ```npx playwright test --grep "@sas_reactions_async_single_user_single_device_delete_json" --workers={WORKER_NUMBER}```


- Example:
```
npx playwright test --grep "@sas_reactions_async_single_user_single_device_create_json" --workers=3
```
- #### Important! Use following tag for running creating test: `@sas_reactions_async_single_user_single_device_create_json`
- #### Important! Use following tag for running deleting test: `@sas_reactions_async_single_user_single_device_delete_json`
- #### Important! Use the number of `workers` equal to the `number of generated test files`
